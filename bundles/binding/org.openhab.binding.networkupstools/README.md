...
where `<name>` is a name you choose to identify a specific UPS device that is managed by NUT servers.
Every UPS is identified by instance name ("ups1" in the example above). The instance name is used in the item definitions.
You can configure any number of UPS devices managed by NUT servers. 

### Item Configuration

The syntax for the binding configuration string is explained here. The general form is:
...
Supported property names differs between UPSes. You can use the `upsc` command to get a list of properties for your ups:
